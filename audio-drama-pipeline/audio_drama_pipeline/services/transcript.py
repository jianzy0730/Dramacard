...
def load_asr_model(model_path: str, compute_type: str = "int8"):
    ...
    try:
        ...
        with temporary_clear_proxy_env():
        ...
        if not is_local_dir and "proxy" in str(exc).lower():
            raise RuntimeError(
                "远程 ASR 模型加载失败：当前环境代理配置不兼容。"
                "我已经自动回避代理重试过，但仍未成功。"
                "请优先使用人工 SRT/TXT，或先把本地 Whisper 模型下载完整。"
            ) from exc
        raise
...
def run_asr(asr_model, audio_path: str, language: str = "", beam_size: int = 5) -> List[TranscriptLine]:
    segments, _info = asr_model.transcribe(
        ...
        beam_size=beam_size,
        vad_filter=True,
        vad_parameters={"min_silence_duration_ms": 250, "speech_pad_ms": 180},
        condition_on_previous_text=True,
        initial_prompt="这是一段中文短剧对白，请尽量准确识别人名、称呼、关系、情绪化台词和口语表达。",
