...
AUDIO_CLEAN_FILTER = ",".join(
    [
        "highpass=f=120",
        "lowpass=f=7600",
        "afftdn=nf=-22",
        "acompressor=threshold=-18dB:ratio=2.5:attack=15:release=180:makeup=2.5",
        "dynaudnorm=f=180:g=12:p=0.9",
    ]
)
...
def extract_audio_to_wav(video_path: str, work_dir: str) -> str:
    ...
    raw_audio_path = os.path.join(work_dir, "audio_track_raw.wav")
    audio_path = os.path.join(work_dir, "audio_track_clean.wav")
    cmd = [
        "ffmpeg",
        "-y",
        "-i",
    ...
    clean_result = subprocess.run(clean_cmd, capture_output=True, text=True, check=False)
